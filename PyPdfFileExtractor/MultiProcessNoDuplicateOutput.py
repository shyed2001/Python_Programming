...
source_folder = Path(r"E:\ZoteroStorage Test\ZoteroRenamedPDF\ZoteroStorageDirs")
destination_folder = Path(r"E:\ZoteroStorage Test\ZoteroRenamedPDF\ALLRenamedPDF")
duplicates_folder = Path(r"E:\ZoteroStorage Test\ZoteroRenamedPDF\DuplicatedPDFrenamed")
...
def extract_pdfs():
    ...
    # Dynamic worker allocation - adjust based on folder count and available memory
    if total_folders < 100:
        # For small collections, use fewer workers to avoid overhead
        worker_percent = 0.5
    elif total_folders > 1000:
        # For very large collections, use more workers
        worker_percent = 0.85
    else:
        # Default for medium collections
        worker_percent = 0.75
    
    # Limit based on available memory (simple heuristic)
    try:
        import psutil
        mem_percent = psutil.virtual_memory().percent
        if mem_percent > 80:
            # High memory usage, reduce worker count
            worker_percent = min(worker_percent, 0.5)
            logger.warning(f"High memory usage detected ({mem_percent}%), reducing worker count")
    except ImportError:
        # psutil not available, continue with default
        pass
    
    max_workers = max(1, int(cpu_count * worker_percent))
        ...
        with tqdm(total=total_folders, desc="Processing folders") as progress_bar:
            for future in concurrent.futures.as_completed(future_to_folder):
                ...
                try:
                    ...
                    # Log status based on result
                    if result["status"] == "processed":
                        if result["pdfs_copied"] > 0:
                            logger.info(f"Copied {result['pdfs_copied']}/{result['pdfs_found']} PDFs from folder '{folder_name}'")
                            if result["pdfs_copied"] < result["pdfs_found"]:
                                logger.warning(f"Failed to copy {result['pdfs_found'] - result['pdfs_copied']} PDFs from '{folder_name}'")
                    elif result["status"] == "permission_error":
                        logger.error(f"Permission error in folder '{folder_name}': {result.get('error', 'Unknown error')}")
                    elif result["status"] == "error" or result["status"] == "scan_error":
                    ...
                    if progress_bar.n % 100 == 0 or progress_bar.n == total_folders:
                        logger.info(f"Progress: {progress_bar.n}/{total_folders} folders processed ({progress_bar.n/total_folders*100:.1f}%)")
                    
                except Exception as e:
                    logger.error(f"Future execution error for folder '{folder_name}': {e}", exc_info=True)
    ...
    for result in results:
        ...
        if result["status"] == "error" or result["status"] == "not_dir" or result["status"] == "scan_error":
            stats["error_folders"] += 1
        elif result["status"] == "permission_error":
            stats["permission_error_folders"] += 1
    ...
    folder_success_rate = ((stats["folders_processed"] - stats["error_folders"] - stats["permission_error_folders"]) / 
                           stats["total_folders"] * 100) if stats["total_folders"] > 0 else 0
    pdf_success_rate = (stats["total_pdfs_copied"] / stats["total_pdfs_found"] * 100) if stats["total_pdfs_found"] > 0 else 0
    
    # Log results
    summary = (
        f"\n===== PDF Extraction Summary =====\n"
        f"Total folders: {stats['total_folders']}\n"
        f"Folders processed: {stats['folders_processed']} ({folder_success_rate:.1f}% success rate)\n"
        f"Empty folders (no PDFs): {stats['empty_folders']}\n"
        f"Problem folders: {stats['error_folders']}\n"
        f"Permission error folders: {stats['permission_error_folders']}\n"
        f"Total PDFs found: {stats['total_pdfs_found']}\n"
        f"Total PDFs copied: {stats['total_pdfs_copied']} ({pdf_success_rate:.1f}% success rate)\n"
    ...
    # Write a simple CSV summary for easy analysis with timestamp
    csv_filename = f"pdf_extraction_results_{timestamp}.csv"
    try:
        with open(csv_filename, "w") as f:
            f.write("Folder,Status,PDFs Found,PDFs Copied,Error\n")
            for result in results:
                error_msg = result.get("error", "").replace(",", ";").replace("\n", " ")
                f.write(f"{result['folder']},{result['status']},{result.get('pdfs_found', 0)},{result.get('pdfs_copied', 0)},\"{error_msg}\"\n")
        logger.info(f"Results CSV file created: {csv_filename}")
        print(f"Results CSV file created: {csv_filename}")
    except Exception as e:
...
def analyze_and_move_duplicates():
    ...
    # Group duplicates by original filename
    duplicate_groups = defaultdict(list)
    for dupe in duplicates:
        original_name = get_original_name(dupe.name)
        duplicate_groups[original_name].append(dupe)
    
    # Count unique documents that have duplicates
    unique_docs_with_dupes = len(duplicate_groups)
    print(f"Found {unique_docs_with_dupes} unique documents with duplicates")
    logger.info(f"Found {unique_docs_with_dupes} unique documents with duplicates")
    
    # Find documents with most duplicates
    if duplicate_groups:
        max_dupes = max(len(dupes) for dupes in duplicate_groups.values())
        docs_with_max_dupes = [orig for orig, dupes in duplicate_groups.items() if len(dupes) == max_dupes]
        print(f"Maximum duplicates for a single document: {max_dupes}")
        print(f"Documents with {max_dupes} duplicates: {', '.join(docs_with_max_dupes[:5])}" + 
              (f" and {len(docs_with_max_dupes) - 5} more..." if len(docs_with_max_dupes) > 5 else ""))
        logger.info(f"Maximum duplicates for a single document: {max_dupes}")
        logger.info(f"Documents with {max_dupes} duplicates: {', '.join(docs_with_max_dupes[:5])}" + 
                  (f" and {len(docs_with_max_dupes) - 5} more..." if len(docs_with_max_dupes) > 5 else ""))
    
    # Move duplicates to separate folder with progress bar
    print(f"Moving {duplicate_count} duplicate files to {duplicates_folder}")
    logger.info(f"Moving {duplicate_count} duplicate files to {duplicates_folder}")
    
    moved_count = 0
    errors = 0
    file_sizes = 0  # Track total size of moved files
    
    # Use tqdm for progress bar
    with tqdm(total=duplicate_count, desc="Moving duplicates") as progress_bar:
        for dupe in duplicates:
            try:
                # Use timestamp and hash based unique name for the destination
                dest_path = generate_unique_name(dupe, duplicates_folder)
                
                # Get file size before moving
                file_size = dupe.stat().st_size
                ...
                if moved_count % 100 == 0:
                    logger.info(f"Moved {moved_count}/{duplicate_count} duplicate files")
                
            except Exception as e:
                logger.error(f"Error moving {dupe}: {e}", exc_info=True)
                errors += 1
                progress_bar.update(1)
    ...
    total_size_mb = file_sizes / (1024 * 1024)
    
    # Print summary with success rate
    success_rate = moved_count / duplicate_count * 100 if duplicate_count > 0 else 0
    ...
    move_summary = (
        ...
        f"Unique documents with duplicates: {unique_docs_with_dupes}\n"
        f"Duplicates moved successfully: {moved_count} ({success_rate:.1f}% success rate)\n"
        f"Total size of moved files: {total_size_mb:.2f} MB\n"
        f"Failed operations: {errors}\n"
        f"Time taken: {elapsed_minutes:.2f} minutes ({elapsed_time:.2f} seconds)\n"
        f"Processing rate: {moved_count/elapsed_minutes:.2f} PDFs/minute\n"
        f"=========================================="
    )
    ...
    if errors > 0:
        print(f"WARNING: {errors} files could not be moved. See log for details.")
        logger.warning(f"{errors} files could not be moved.")
    
    # Calculate how many PDFs remain in the destination folder
    remaining_pdfs = len(list(destination_folder.glob("*.pdf")))
    print(f"Remaining PDFs in destination folder: {remaining_pdfs}")
    logger.info(f"Remaining PDFs in destination folder: {remaining_pdfs}")
    
    # Write duplicate summary to CSV with timestamp
    csv_filename = f"duplicate_pdfs_summary_{timestamp}.csv"
    try:
        with open(csv_filename, "w", encoding="utf-8") as f:
            f.write("Original Filename,Duplicate Count,Moved Filenames\n")
            for original, dupes in duplicate_groups.items():
                dupe_names = "|".join(d.name for d in dupes)
                f.write(f"\"{original}\",{len(dupes)},\"{dupe_names}\"\n")
        print(f"Created duplicate summary CSV: {csv_filename}")
        logger.info(f"Created duplicate summary CSV: {csv_filename}")
    except Exception as e:
        logger.error(f"Error creating CSV summary: {e}", exc_info=True)
