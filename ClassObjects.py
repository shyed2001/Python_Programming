
print("""
print('''

CLLASSES
CLLASSES
and
OBJECTS
and
OBJECTS

''')

print('''
Create your own data type to represt all type of data
With a class define your own data type
 ''')

print('''
class students: # Tocreate the student class, then use str, int,
    float, bulleans to map out the class
 ''')
class student:  # Tocreate the student class
    def __init__(self, name, major, gpa, is_on_prob ):# Initialised function.
 # this is the student data type
        self.name = name
        self.major = major
        self.gpa = gpa
        self.is_on_prob = is_on_prob  # student class defined.
# This is a template

print('''
An Object is an actual student,
so that is an student with a name, a major, a gpa
 ''')

""")



print(''' TO create the actual object ant print or use ethat data ty class
we have to code

from student import student

student1 = student(" Jim", "CSE", 3.5, False)
student2 = student(" kim", "eee", 2.5, True)
print(student1)
print(student1.name)
print(student2.name)
print(student2.gpa)

 ''')

from student import student

student1 = student(" Jim", "CSE", 3.5, False)
student2 = student(" kim", "eee", 2.5, True)
print(student1)
print(student1.name)
print(student2.name)
print(student2.gpa)

